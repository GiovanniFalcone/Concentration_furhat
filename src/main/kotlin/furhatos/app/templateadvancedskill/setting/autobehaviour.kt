...
val UniversalFallbackBehaviour = partialState {
    ...
    onNoResponse {
        furhat.say("Scusami non ho sentito. ")
        reentry()
    }
    onNoResponse() {
        noInput++
        when (noInput) {
            0 -> furhat.say("Scusami non ho sentito. ")
            else -> furhat.say("Scusami continuo a non sentirti. ")
        }
        reentry()
    }
    onResponse {
        noMatch++
        when (noMatch) {
            0 -> furhat.say("Scusami, non ho capito. ")
            else -> furhat.say("Scusami, continuo a non capire. ")
        }
        reentry()
    }
    // Override default error messages on internet failure
    onResponseFailed {
        furhat.say("Scusami, sto riscontrando problemi di connessione.")
        reentry()
    }
    onNetworkFailed {
        furhat.say("Scusami, sto riscontrando problemi di connessione.")
